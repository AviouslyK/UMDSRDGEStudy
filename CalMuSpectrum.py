...
#Read in DataSet
df        = pd.read_csv("LDR_irr_10data.csv", sep=",")
Waves     = df.Waves
Kak_a     = df.T_K_a
Kak_c     = df.T_K_c
Andres_a  = df.T_A_a
...
TransE = np.mean(np.array([Kak, Andres, Andres2, Andres3, Avi, Avi2]), axis=0)
...
for i in range (len(Waves)):
    ...
    for mu in np.arange(start,stop,step):
         #10 mm fit function
        Diff[k]  = abs(E_Meas - Eff10(mu));
        #print('E_Meas[ ]= ', E_Meas, ' and Eff10(',mu,'mm)= ', Eff10(mu))
        #6 mm fit function
        #Diff[k]  = abs(E_Meas - Eff6(mu));
...
f = open('Muspec_10mm_LDR_psigma.txt','w')
